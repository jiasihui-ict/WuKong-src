...
sealed class BankedStage1Out(implicit val cacheConfig: BankedCacheConfig)
  ...
  //jsh加入isread的信号，便于mmio的read的标识
  val reqisRead = Output(Bool())
}
// meta read
class BankedCacheStage1(implicit val cacheConfig: BankedCacheConfig)
  ...
  //jsh ISread信号
  io.out(0).bits.reqisRead := io.in(0).bits.isRead()
  io.out(1).bits.reqisRead := io.in(1).bits.isRead()
  ...
  //jsh 增加S1的mmiostall信号，之前的问题在于只涉及了在S2阶段的stall，理论上应该在S1阶段发现是mmioread就应该及时发出阻塞，加入后，和S2阶段的或上就行
  //val MMioReadStallS1 = WireInit(false.B)
  //MMioReadStallS1 := io.in(0).bits.isRead() && io.out(0).bits.mmio && io.out(0).fire        //inReadMMIOState || reqIsRead0 && mmio0raqfire
  //BoringUtils.addSource(MMioReadStallS1,"MMioReadStallS1")
...
sealed class BankedCacheStage2(implicit val cacheConfig: BankedCacheConfig)
  ...
  class BankedCacheStage2IO extends Bundle {
    ...
    val mmio = new SimpleBusUC   // line0 MMIO
    val mmio1 = new SimpleBusUC  // line1 MMIO, newly added
    //later release
  ...
  for(i <- 0 until 2){
    ...
    miss(i)    := !(hitVec(i).orR) && io.in(i).valid && !mmio(i) 
...
//    assert(!(mmio && hit), "MMIO request should not hit in cache")
  ...
  // mmio
  // activeMMIOLine: false.B -> line0, true.B -> line1.
  // line0 still has higher priority when both line0 and line1 request MMIO.
  val activeMMIOLine = RegInit(false.B)
  val mmioLine1Selected = activeMMIOLine

  io.mmio.req.bits := req(0)
  io.mmio1.req.bits := req(1)

  io.mmio.req.valid := (state === s_mmioReq) && !mmioLine1Selected
  io.mmio1.req.valid := (state === s_mmioReq) && mmioLine1Selected

  io.mmio.resp.ready := !mmioLine1Selected
  io.mmio1.resp.ready := mmioLine1Selected

  val mmioReqFire = Mux(mmioLine1Selected, io.mmio1.req.fire(), io.mmio.req.fire())
  val mmioRespFire = Mux(mmioLine1Selected, io.mmio1.resp.fire(), io.mmio.resp.fire())

  // Hold line1 MMIO response after external resp.fire().
  // Without this latch, state moves from s_mmioResp to s_wait_resp,
  // but io.mmio1.resp.valid may already be gone, so io.out(1).fire() can never happen.
  val mmio1RespHold = RegInit(false.B)
  val mmio1RespData = RegInit(0.U(DataBits.W))
  val mmio1WaitResp = mmioLine1Selected && mmio1RespHold

  when(mmioLine1Selected && io.mmio1.resp.fire()) {
    mmio1RespHold := true.B
    mmio1RespData := io.mmio1.resp.bits.rdata
  }.elsewhen(mmio1RespHold && io.out(1).fire()) {
    mmio1RespHold := false.B
  }
  ...
  switch(state) {
    is(s_idle) {
      ...
      when(((miss(0) || miss(1)) && !storeHit || mmio(0) || mmio(1)) && !io.flush || mmioStorePending) {
        ...
        // Select MMIO owner before entering s_mmioReq.
        // Priority: MMIO store/line0 > line0 load/store > line1.
        when(mmioStorePending) {
          activeMMIOLine := false.B
        }.elsewhen(mmio(0)) {
          activeMMIOLine := false.B
        }.elsewhen(mmio(1)) {
          activeMMIOLine := true.B
        }
    ...
    is(s_mmioResp) {
      when(mmioRespFire) {
        // line1 MMIO response is latched above, then delivered to io.out(1) in s_wait_resp.
        // line0 keeps the original readmmioDelay0 behavior.
        state := Mux(
          mmioLine1Selected,
          s_wait_resp,
          Mux(readmmioDelay0, s_idle, Mux(mmio(0), s_wait_resp, s_idle))
        )
      }
    }
    ...
    is(s_wait_resp) {
      when(io.out(0).fire() || io.out(1).fire() ) {
        state := s_idle
        activeMMIOLine := false.B
      }.elsewhen(!mmio1WaitResp && bank_conflict){
        state := s_idle
      }.elsewhen(!mmio1WaitResp && both_miss){
        state := s_idle
      }
    }
  }
  ...
  miss(0)    := !(hitVec(0).orR) && io.in(0).valid && !process_channel1 && !mmio(0)
  ...
  io.out(1).valid := Mux(
    mmio(1) || mmio1RespHold,
    mmioLine1Selected && mmio1RespHold && (state === s_wait_resp),
    io.in(1).valid && Mux(
      hit(1),  //
      true.B,
  ...
  //io.out(1).bits.rdata := Mux(hit(1), dataRead(1), inRdataRegDemand)
    ...
    io.out(1).bits.rdata := Mux(
      mmio1RespHold && mmioLine1Selected,
      mmio1RespData,
      Mux(hit(1), dataRead(1), inRdataRegDemand)
    )
  ...
  io.in(0).ready := io.out(0).ready && (state === s_idle && !(miss.asUInt.orR()) && !mmio(0) || io.release_later)
  io.in(1).ready := io.out(1).ready && (state === s_idle && !(miss.asUInt.orR()) && !mmio(1) || io.release_later)
...
// MMIO terminal arbiter.
// Default mode is line0. line0 has higher priority.
// If line1 wins, it keeps the output until its response returns.
class BankedMMIO2to1Arb(implicit val cacheConfig: BankedCacheConfig)
  extends BankedCacheModule {
  class BankedMMIO2to1ArbIO extends Bundle {
    val in0 = Flipped(new SimpleBusUC) // line0, high priority
    val in1 = Flipped(new SimpleBusUC) // line1
    val out = new SimpleBusUC
  }

  val io = IO(new BankedMMIO2to1ArbIO)

  val s_line0 :: s_line1 :: Nil = Enum(2)
  val state = RegInit(s_line0)
  val busy = RegInit(false.B)

  val chooseLine1 = !busy && !io.in0.req.valid && io.in1.req.valid
  val useLine1 = (state === s_line1) || chooseLine1

  io.out.req.valid := Mux(useLine1, io.in1.req.valid, io.in0.req.valid)
  io.out.req.bits  := Mux(useLine1, io.in1.req.bits,  io.in0.req.bits)

  io.in0.req.ready := !useLine1 && io.out.req.ready
  io.in1.req.ready :=  useLine1 && io.out.req.ready

  io.out.resp.ready := Mux(state === s_line1, io.in1.resp.ready, io.in0.resp.ready)

  io.in0.resp.valid := (state === s_line0) && io.out.resp.valid
  io.in0.resp.bits  := io.out.resp.bits

  io.in1.resp.valid := (state === s_line1) && io.out.resp.valid
  io.in1.resp.bits  := io.out.resp.bits

  when(!busy && io.out.req.fire()) {
    busy := true.B
    state := Mux(useLine1, s_line1, s_line0)
  }

  when(busy && io.out.resp.fire()) {
    busy := false.B
    state := s_line0
  }
}
...
class DCache(implicit val cacheConfig: BankedCacheConfig)
    ...
    val mmioArb = Module(new BankedMMIO2to1Arb)
    mmioArb.io.in0 <> s2.io.mmio
    mmioArb.io.in1 <> s2.io.mmio1
    io.mmio <> mmioArb.io.out
