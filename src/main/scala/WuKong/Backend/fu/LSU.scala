...
//  }
  ...
  //JSH 直接从LSU这里就判断是否是mmioreadstall
  val MMioReadStall = WireInit(false.B)
  val MMioReadStall0 = WireInit(false.B)
  val MMioReadStall1 = WireInit(false.B)
  val readmmioDelay0 = RegInit(false.B)
  val readmmioDelay1 = RegInit(false.B)
  val reqIsRead0 = WireInit(false.B)
  val reqIsRead1 = WireInit(false.B)
  val LSUreadmmio0 = WireInit(false.B)
  val LSUreadmmio1 = WireInit(false.B)
  val mmio0raqfire = WireInit(false.B)
  val mmio1raqfire = WireInit(false.B)
  //JSH mmio0
  LSUreadmmio0 := AddressSpace.isMMIO(io.dmem(0).req.bits.addr) && reqIsRead0
  reqIsRead0 :=  (io.dmem(0).req.bits.cmd === 0.U(4.W))
  mmio0raqfire := io.dmem(0).req.fire
  when(LSUreadmmio0 && mmio0raqfire && !readmmioDelay0){
      readmmioDelay0 := true.B}
  when(io.dmem(0).resp.fire() && readmmioDelay0){
      readmmioDelay0 := false.B}
  //MMioReadStall := (readmmioDelay0  ||  LSUreadmmio && mmio0raqfire) && !io.dmem(0).resp.fire()
  MMioReadStall0 := readmmioDelay0 && !io.dmem(0).resp.fire()

  //mmio1
  LSUreadmmio1 := AddressSpace.isMMIO(io.dmem(1).req.bits.addr) && reqIsRead1
  reqIsRead1 :=  (io.dmem(1).req.bits.cmd === 0.U(4.W))
  mmio1raqfire := io.dmem(1).req.fire
  when(LSUreadmmio1 && mmio1raqfire && !readmmioDelay1){
    readmmioDelay1 := true.B}
  when(io.dmem(1).resp.fire() && readmmioDelay1){
    readmmioDelay1 := false.B}
  MMioReadStall1 := readmmioDelay1 && !io.dmem(1).resp.fire()

  MMioReadStall := MMioReadStall1 || MMioReadStall0

  BoringUtils.addSource(MMioReadStall,"MMioReadStall")
  val isMMIOread = LSUreadmmio0 && mmio0raqfire || LSUreadmmio1 && mmio1raqfire
  BoringUtils.addSource(isMMIOread,"isMMIOread")
