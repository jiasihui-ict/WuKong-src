...
//在这里引入了edirectFlushSave信号，在stall期间保存Redirect相关信号，在stall结束后使用
 ...
 when(Redirect8.valid && pipeOut(8).valid && !pipeInvalid(10)) {
  ...
  //LSU flush,JSH修改，让LSU内部不会invalid，所有都等LSU结果返回之后，stall拉低之后统一冲刷，只在MMIO的时候增加判定，其他不变
    ...
    val isMMIOread = WireInit(false.B)
    BoringUtils.addSink(isMMIOread,"isMMIOread")
    LSU.io.invalid(0) := Mux(MMioReadStall || isMMIOread , false.B ,ALU_6.io.redirect.valid || ALU_7.io.redirect.valid)
    LSU.io.invalid(1) := Mux(MMioReadStall || isMMIOread , false.B ,ALU_6.io.redirect.valid || ALU_7.io.redirect.valid)
