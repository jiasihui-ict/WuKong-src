...
import WuKong.Backend.fu.CSROpType
import WuKong.{HasCoreParameter, HasCoreConst}
...
object FuType extends HasCoreConst {
  def num = 5
  def alu = "b000".U
  def lsu = "b001".U
  def mdu = "b010".U
...
object Instructions extends HasInstrType with HasCoreParameter {
  def NOP = 0x00000013.U
  val DecodeDefault = List(InstrN, FuType.csr, CSROpType.jmp)
  def DecodeTable = RVIInstr.table ++ CoreTrap.table ++
...
object CInstructions extends HasInstrType with HasCoreParameter{
  def NOP = 0x00000013.U
