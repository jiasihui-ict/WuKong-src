...
import WuKong.isa.{Priviledged, RV32I_BRUInstr, RVCInstr}
import chisel3._
...
import WuKong.isa.{HasInstrType, FuType, CInstructions, SrcType, Instructions}
import WuKong.CoreTrap
import top.WuKongConfig

class Decoder(implicit val p: WuKongConfig) extends CoreModule with HasInstrType {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new CtrlFlowIO))
    val out = Decoupled(new DecodeIO)
    val isWFI = Output(Bool()) // require CoreSim to advance mtime when wfi to reduce the idle time in Linux
  ...
  io.out.bits.ctrl.isCoreTrap := (instr(31,0) === CoreTrap.TRAP) && io.in.valid
  ...
  io.out.bits.ctrl.isCoreTrap := (instr === CoreTrap.TRAP) && io.in.valid
...
class IDU(implicit val p: WuKongConfig) extends CoreModule with HasInstrType {
  val io = IO(new Bundle {
